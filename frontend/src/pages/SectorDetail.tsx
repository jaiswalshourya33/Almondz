  ...
  if (!sector) {
    return (
      <div className="min-h-screen bg-[#F1F3F5] pt-32 pb-20 text-center">
        <h2 className="text-3xl font-serif text-[#2B4A6D]">Sector Not Found</h2>
        <p className="text-sm text-[#2B4A6D]/70 mt-2">The requested sector could not be located.</p>
        <Link to="/sectors" className="mt-6 inline-block bg-[#2B4A6D] hover:bg-[#3E4C60] text-white px-6 py-3 text-xs font-mono font-bold tracking-widest uppercase shadow-md hover:shadow-lg transition-all duration-300 rounded-md">
          Back to Sectors
        </Link>
      </div>
    );
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 pb-4 gap-6">
              <div>
                ...
                <h2 className="text-2xl sm:text-3xl font-serif font-bold text-[#2B4A6D] leading-snug">
                  <span className="block">Representative Projects in</span>
                  <span className="block text-[#2B4A6D]">{sector.title}</span>
                </h2>
                <p className="text-xs text-[#2B4A6D]/65 mt-2 font-medium">
              ...
              <div className="flex items-center gap-3 self-start lg:self-auto flex-wrap">
                ...
                <div className="hidden sm:flex items-center gap-1.5 pl-1">
                  <button
                    ...
                    className="w-8 h-8 rounded-full bg-white border border-gray-300 hover:border-[#2B4A6D] hover:bg-[#2B4A6D] hover:text-white flex items-center justify-center text-gray-700 transition-all duration-200 shadow-2xs cursor-pointer focus:outline-none"
            ...
            {filteredProjects.length === 0 ? (
              <div className="text-center py-16 bg-white border border-gray-200 rounded-sm">
                ...
                <button
                  ...
                  className="mt-4 px-4 py-2 bg-[#2B4A6D] hover:bg-[#3E4C60] text-white text-xs font-semibold uppercase tracking-wider transition-colors rounded-sm"
                >
                  View All Projects
                </button>
              </div>
                ...
                {filteredProjects.map((proj) => (
                  ...
                  >
                    ...
                    <div className="flex-1 flex items-center justify-center px-2.5 py-2.5">
                      <h3
                        className="text-[11px] font-semibold text-center text-[#2B4A6D] leading-snug line-clamp-2 group-hover:text-[#3E4C60] transition-colors"
                        title={proj.title}
                      >
                        {proj.title}
                      </h3>
