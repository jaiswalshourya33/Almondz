  ...
  if (!item) {
    return (
      <div className="min-h-screen bg-[#F1F3F5] pt-32 pb-20 text-center">
        <h2 className="text-3xl font-serif text-[#2A4C72]">Page Not Found</h2>
        <p className="text-sm text-[#2A4C72]/70 mt-2">The requested corporate governance page could not be located.</p>
        <Link
          to="/"
          className="mt-6 inline-block bg-[#2A4C72] hover:bg-[#3E4C60] text-white px-6 py-3 text-xs font-mono font-bold tracking-widest uppercase shadow-md hover:shadow-lg transition-all duration-300 rounded-md"
        >
          Back to Home
        </Link>
      </div>
      ...
      <section ref={sectionRef} className="about-subnav-section py-20 bg-[#F1F3F5]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div ref={headerRef} className="about-subnav-header text-center mb-14">
            ...
            <h2 className="text-3xl font-serif font-bold text-[#2A4C72] mt-1">{item.navLabel}</h2>
            {item.intro && (
              <p className="text-sm text-[#2A4C72]/70 leading-relaxed max-w-3xl mx-auto mt-4">{item.intro}</p>
            )}
          </div>
          ...
          {directorResignations.length > 0 && (
            <div ref={cardsRef} className="director-resignations">
              <div className="flex items-center gap-4 mb-12">
                <span className="shrink-0 w-11 h-11 rounded-xl bg-[#2A4C72] flex items-center justify-center text-[#D6C489]">
                  <UserMinus className="w-5 h-5" />
                </span>
                <div>
                  <h3 className="text-xl sm:text-2xl font-serif font-bold text-[#2A4C72] leading-tight">
                    Resignation of Director
                  </h3>
                  <p className="text-[11px] font-mono uppercase tracking-[0.22em] text-[#2A4C72]/45 mt-1">
                ...
                <div className="flex flex-col gap-12">
                  {directorResignations.map((year) => (
                    <div key={year.period} className="gov-timeline-year relative pl-10 sm:pl-14">
                      ...
                      <span
                        aria-hidden="true"
                        className="absolute left-[3px] top-1 w-3.5 h-3.5 rounded-full bg-[#2A4C72] ring-4 ring-[#F1F3F5] shadow-[0_0_0_1px_rgba(164,144,80,0.5)]"
                      />

                      {/* year heading */}
                      <div className="flex items-baseline justify-between gap-4 border-b border-[#A49050]/25 pb-3 mb-3">
                        <h4 className="text-2xl sm:text-[30px] font-serif font-bold text-[#2A4C72] leading-none">
                          {year.period}
                        </h4>
                      ...
                      <ul className="flex flex-col">
                        {year.directors.map((name, j) => (
                          ...
                          >
                            ...
                            <span
                              className="shrink-0 w-9 h-9 rounded-full border border-[#A49050]/30 bg-white flex items-center justify-center text-[11px] font-bold text-[#2A4C72] group-hover/row:border-[#D96B33]/45 group-hover/row:text-[#D96B33] transition-colors"
                              aria-hidden="true"
                            >
                              {initialsOf(name)}
                            </span>
                            <span className="min-w-0 flex-1 text-[15px] sm:text-base font-serif text-[#2A4C72]">
                              {name}
                            </span>
                            <span className="shrink-0 inline-flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-[0.18em] text-[#2A4C72]/40">
            ...
            <div ref={cardsRef} className="general-meetings flex flex-col gap-20">
              {generalMeetings.map((group) => (
                <div key={group.category} className="gm-category">
                  <div className="flex items-center gap-4 mb-12">
                    <span className="shrink-0 w-11 h-11 rounded-xl bg-[#2A4C72] flex items-center justify-center text-[11px] font-mono font-bold tracking-[0.15em] text-[#D6C489]">
                      {group.abbr}
                    </span>
                    <div>
                      <h3 className="text-xl sm:text-2xl font-serif font-bold text-[#2A4C72] leading-tight">
                          ...
                          <ul className="flex flex-col">
                            {year.notices.map((notice, j) => (
                              ...
                              >
                                ...
                                <span
                                  className="shrink-0 w-9 h-9 rounded-full border border-[#A49050]/30 bg-white flex items-center justify-center text-[#2A4C72] group-hover/row:border-[#D96B33]/45 group-hover/row:text-[#D96B33] transition-colors"
                                  aria-hidden="true"
                                >
              ...
              {documents.map((doc) => (
                ...
                >
                  <div className="flex flex-col gap-4">
                    ...
                    <h3 className="text-xl font-serif font-bold text-[#2A4C72]">{doc.title}</h3>
                    <p className="text-xs text-[#2A4C72]/70 leading-relaxed">{doc.summary}</p>
                  </div>
          ...
          {documents.length === 0 &&
            ...
            directorResignations.length === 0 &&
            generalMeetings.length === 0 && (
              <p className="text-center text-sm text-[#2A4C72]/70 leading-relaxed">
