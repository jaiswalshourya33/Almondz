...
export const ServiceCard: React.FC<ServiceCardProps> = ({ service }) => {
  return (
    <Link 
      ...
      className="service-parallax-card group bg-white border border-[#A49050]/30 p-6 flex flex-col justify-between shadow-sm hover:shadow-xl hover:border-[#A49050] transition-all duration-500"
    >
      <div className="service-card__body flex flex-col gap-4">
        <div className="service-card__top flex justify-end items-start">
          <span className="text-[10px] font-mono font-bold tracking-widest text-[#18253A] uppercase">
            CONSULTANCY
          </span>
        </div>

        <h3 className="service-card__title text-lg font-serif text-[#18253A] group-hover:text-[#A49050] transition-colors leading-snug">
          {service.title}
        </h3>

        <p className="service-card__copy text-sm text-[#18253A] leading-relaxed">
          {service.shortDesc}
        </p>

        <div className="service-card__list pt-4 border-t border-[#A49050]/20">
          <span className="text-[10px] font-mono font-bold tracking-widest text-[#18253A] uppercase">Key deliverables</span>
          <ul className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
            {service.deliverables.map((item, idx) => (
              <li key={idx} className="flex items-start gap-2 text-xs text-[#18253A]">
      ...
      <div className="service-card__footer pt-6 mt-4 border-t border-gray-100 flex items-center justify-between">
        <span className="text-xs font-mono font-bold text-[#18253A] group-hover:text-[#A49050] transition-colors">
          VIEW CAPABILITIES
        </span>
