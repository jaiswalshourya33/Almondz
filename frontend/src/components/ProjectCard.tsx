...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, onOpenDetails }) => {
  ...
  const statusBadgeStyles: Record<Project["status"], { bg: string; dot: string; text: string }> = {
    ...
    "Ongoing": {
      bg: "bg-blue-50 border-blue-200/80 text-[#2A4C72]",
      dot: "bg-blue-600",
      text: "Ongoing"
    },
    "Completed": {
    ...
    <div className="group project-card bg-white border border-gray-200/80 rounded-xl overflow-hidden flex flex-col justify-between shadow-[0_2px_10px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_20px_rgba(62,76,96,0.14)] hover:border-[#3E4C60] transition-all duration-300 hover:-translate-y-0.5">
      ...
      <div className="p-4 sm:p-5 flex flex-col justify-between flex-1 gap-3 min-h-[140px]">
        ...
        <div>
          ...
          <h3
            className="mt-2.5 text-xs sm:text-[13px] font-serif font-medium text-[#2A4C72] group-hover:text-[#3E4C60] transition-colors leading-snug line-clamp-3"
            title={project.title}
          >
            {project.title}
          </h3>
        ...
        <div className="pt-3 mt-auto">
          {onOpenDetails ? (
            <button
              type="button"
              onClick={handleOpenDetails}
              className="w-full py-2 px-3 bg-[#2A4C72] hover:bg-[#3E4C60] hover:text-[#D6C489] text-white text-[10.5px] font-bold tracking-wider uppercase rounded-md transition-all duration-200 flex items-center justify-center gap-1 shadow-xs hover:shadow-sm cursor-pointer active:scale-[0.99]"
            >
            ...
            <Link
              to="/projects"
              className="w-full py-2 px-3 bg-[#2A4C72] hover:bg-[#3E4C60] hover:text-[#D6C489] text-white text-[10.5px] font-bold tracking-wider uppercase rounded-md transition-all duration-200 flex items-center justify-center gap-1 shadow-xs hover:shadow-sm active:scale-[0.99]"
            >
              <span>View Details</span>
              <ArrowRight className="w-3 h-3 group-hover:translate-x-0.5 transition-transform" />
            </Link>
